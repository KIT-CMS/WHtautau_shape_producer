...
if [[ $MODE == "IMPACTS" ]]; then
    ...
    do
        ...
        # echo "huuu"
        # echo $WORKSPACE
        combineTool.py -M Impacts -d $WORKSPACE -m 125 --doInitialFit --setParameters r_WH=1.0 --setParameterRanges r_WH=-5,5 --redefineSignalPOIs r_WH -v1 --parallel 16 

        combineTool.py -M Impacts -d $WORKSPACE -m 125 --doFits --setParameters r_WH=1.0 --setParameterRanges r_WH=-5,5 --redefineSignalPOIs r_WH -v1 --parallel 16 

        combineTool.py -M Impacts -d $WORKSPACE -m 125 -o sm_mc_${ERA}_${CHANNEL}_impacts.json --setParameters r_WH=1.0 --setParameterRanges r_WH=-5,5 --redefineSignalPOIs r_WH -v1
        ...
        combineTool.py -M Impacts -d $WORKSPACE -m 125 -o sm_mc_${ERA}_${CHANNEL}_impacts.json --setParameters r_WH=1.0 \
            --setParameterRanges r_WH=-5,5 \
            --redefineSignalPOIs r_WH -v1
        plotImpacts.py -i $DATACARD_OUTPUT/sm_mc_${ERA}_${CHANNEL}_impacts.json -o sm_mc_${ERA}_${CHANNEL}_impacts 
...
if [[ $MODE == "UNC_SPLIT" ]]; then
    ...
    for POI in ${POIS[@]}; do
        combine -M MultiDimFit $DATACARD_OUTPUT/workspace.root -n .snapshot -m 125 --saveWorkspace --redefineSignalPOIs $POI --setParameters r_WH=1.0 --robustFit 1 \
                --X-rtd MINIMIZER_analytic \
                --cminDefaultMinimizerStrategy 0 \

        combine -M MultiDimFit higgsCombine.snapshot.MultiDimFit.mH125.root -n .nominal -m 125 --algo grid --snapshotName MultiDimFit --redefineSignalPOIs $POI --points 20 --setParameters r_WH=1.0 --robustFit 1 \
                --X-rtd MINIMIZER_analytic \
                --cminDefaultMinimizerStrategy 0 \
        ...
        combine -M MultiDimFit higgsCombine.snapshot.MultiDimFit.mH125.root -n .freezeall -m 125 --algo grid --freezeParameters allConstrainedNuisances --snapshotName MultiDimFit --redefineSignalPOIs $POI --points 20 --setParameters r_WH=1.0 --robustFit 1 \
                --X-rtd MINIMIZER_analytic \
                --cminDefaultMinimizerStrategy 0 \
...
############
outputname="scan_WHincl_${ERA}_${CHANNEL}"
...
combineTool.py -M FitDiagnostics \
    ...
    -n myfit \
    --cminDefaultMinimizerStrategy 0
python CMSSW_11_3_4/src/HiggsAnalysis/CombinedLimit/data/tutorials/longexercise/diffNuisances.py fitDiagnostics.Test.root -a -g correlations.root
done 
done 
fi