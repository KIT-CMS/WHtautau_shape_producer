...
ROOT.gInterpreter.Declare(
    ...
    float tau_fakerate(const float &njets, const int &dm, const std::string &wp_vs_jets, const std::string &wp_vs_mu, const std::string &wp_vs_ele, const std::string &corr_file, const std::string &shift) {
        auto cset = correction::CorrectionSet::from_file(corr_file);
        float sf;
        auto ev = cset->at("jet_to_tau_fakerate");
        if (njets>-0.0001&&dm>-0.0001) {
        sf = ev->evaluate({wp_vs_jets, wp_vs_mu, wp_vs_ele, dm, njets, shift});
        }
        else {
        sf = 1;
        }
    ...
    float lep_fakerate(const float &njets, const std::string &corr_file, const std::string &shift) {
        float sf;
        auto cset = correction::CorrectionSet::from_file(corr_file);
        auto ev = cset->at("jet_to_lep_fakerate");
        if (njets>-0.0001) {
        sf = ev->evaluate({"Tight",njets, shift});
        }
        else {
        sf = 1;
        }
...
def build_rdf(
    ...
    if channel in ["emt", "mmt"]:
        rdf = rdf.Define(
            "tau_closure_correction_njets_Era",
            '(float) tau_fakerate(njets, decaymode_3, "{wp_vs_jets}", "{wp_vs_mu}", "{wp_vs_ele}", "{corr_file}", "nom")'.format(
                wp_vs_jets=wp_vs_jets,
                wp_vs_mu=wp_vs_mu,
                wp_vs_ele=wp_vs_ele,
        ...
        rdf = rdf.Define(
            "lep_closure_correction_njets_Era",
            '(float) lep_fakerate(njets,"{corr_file}", "nom")'.format(
