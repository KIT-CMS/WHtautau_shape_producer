...
#jet to electron fakerate shapes
  ...
  do
      ...
      OUTPUT_FILE="${OUTPUT_DIR}/${FILENAME}"
    #  if ! [ -f "${OUTPUT_FILE}.root" ]; then 
      python shapes/produce_shapes_fakerate.py --channels ${CHANNEL} --output-file ${OUTPUT_FILE} --directory ${NTUPLE_PATH}  --era ${ERA} --num-processes 2 --num-threads 2 --optimization-level 1 --control-plots --ntuple_type crown --${CHANNEL}-friend-directory ${FRIEND_PATH}/crosssection --skip-systematic-variations --control-plot-set pt_3 --wp_vs_jet Tight --wp_vs_jet_tight Tight --wp_vs_mu Tight --wp_vs_ele Tight --decay_mode 0 --id_wp_ele 1 --id_wp_mu $IDWP_MU --xrootd
      #fi
  ...
  OUTPUT_FILE="${OUTPUT_DIR}/${FILENAME}"
  # if ! [ -f "${OUTPUT_FILE}.root" ]; then 
  python shapes/produce_shapes_fakerate.py --channels ${CHANNEL} --output-file ${OUTPUT_FILE} --directory ${NTUPLE_PATH}  --era ${ERA} --num-processes 2 --num-threads 2 --optimization-level 1 --control-plots --ntuple_type crown --${CHANNEL}-friend-directory ${FRIEND_PATH}/crosssection ${FF_FRIEND} ${CLOSURE_FRIEND} ${MET_CLOSURE_FRIEND} --skip-systematic-variations --control-plot-set pt_3,njets,met,eta_3 --wp_vs_jet Tight --wp_vs_jet_tight Tight --wp_vs_mu Tight --wp_vs_ele Tight --decay_mode 0 --id_wp_ele ${IDWP_ELE} --id_wp_mu 1 --xrootd --closure_corr
  ...
  FILENAME="id_wp_mu_${IDWP_MU}"
  OUTPUT_FILE="${OUTPUT_DIR}/${FILENAME}"
  python shapes/produce_shapes_fakerate.py --channels ${CHANNEL} --output-file ${OUTPUT_FILE} --directory ${NTUPLE_PATH}  --era ${ERA} --num-processes 2 --num-threads 2 --optimization-level 1 --control-plots --ntuple_type crown --${CHANNEL}-friend-directory ${FRIEND_PATH}/crosssection ${FF_FRIEND} ${CLOSURE_FRIEND} ${MET_CLOSURE_FRIEND} --skip-systematic-variations --control-plot-set pt_3,njets,met,eta_3 --wp_vs_jet Tight --wp_vs_jet_tight Tight --wp_vs_mu Tight --wp_vs_ele Tight --decay_mode 0 --id_wp_ele 1 --id_wp_mu $IDWP_MU --xrootd --closure_corr
  bash shapes/do_estimations.sh ${ERA} ${OUTPUT_FILE}.root 0
fi