...
def parse_args():
    ...
    parser.add_argument(
        "--datacard_name",
        dest="datacard_name",
        help="datacard to use for the fit",
        default="DEFAULT",
    )
    parser.add_argument(
        "--output_name",
        dest="output_name",
        help="txt file where fit results are saved",
        default="DEFAULT",
    )
    parser.add_argument(
        "--sanity_check",
        dest="sanity_check",
        help="flag for additional fit sanity checks",
        default=False,
    )
    parser.add_argument("--output", help="path to the output datacard")
    parser.add_argument(
        "--freeze_nuisances",
        dest="freeze_nuisances",
        help="freeze systematic uncertainties",
        default=False,
    )
...
def main(args):
    print("Datacard name    = {}".format(args.datacard_name))
    print("Output name      = {}".format(args.output_name))
    print("Do santy checks  = {}".format(args.sanity_check))
    print("Freeze nuisances = {}".format(args.freeze_nuisances))
    # print("Combine name     = {}".format(args.combine_name))

    # Exceptions
    if args.datacard_name == "DEFAULT":
    ...
    datacard_name = args.datacard_name
    output_name = args.output_name

    sanity_check = False
    if args.sanity_check == "True" or args.sanity_check == "1":
    ...
    nuisances = ""
    if (
        args.freeze_nuisances == "True"
        or args.freeze_nuisances == "1"
        or args.freeze_nuisances == "all"
    ):
    ...
    if args.freeze_nuisances == "r_higgs":
        nuisances = "--freezeParameters r_higgs"

    # combine_name = ""
    # if args.combine_name != "DEFAULT":
    ...
    combine_command = "combine \
                    ...
                    > {1} \
                    ".format(
        datacard_name, output_name, nuisances
    )
    ...
    combine_command_WH = "combine \
                        ...
                        > {1} \
                        ".format(
        datacard_name, output_name.replace(".txt", "_WH_strength.txt"), nuisances
    )
    ...
    # if (args.freeze_nuisances) == "1" or (args.freeze_nuisances) == "True":
    ...
    if sanity_check != False:
        ...
        if "ML" in sanity_check:
            ...
            move_command = "mv higgsCombineTest.MultiDimFit.mH120.root {}".format(
                ML_output_name
            )
            print(move_command)
            os.system(move_command)
        ...
        if "rA_scan" in sanity_check:
            ...
            move_command = "mv higgsCombine_r_A_scan.MultiDimFit.mH120.root {}".format(
                rA_output_name
            )
            print(move_command)
            os.system(move_command)
