...
if [[ $MODE == "IMPACTS" ]]; then
    ...
    do
        ...
        echo "huuu"
        echo $WORKSPACE
        combineTool.py -M Impacts -d $WORKSPACE -m 125 --doInitialFit -t -1 --setParameters r_WH=1.0 --setParameterRanges r_WH=-5,5 --redefineSignalPOIs r_WH --parallel 16 

        combineTool.py -M Impacts -d $WORKSPACE -m 125 --doFits -t -1 --setParameters r_WH=1.0 --setParameterRanges r_WH=-5,5 --redefineSignalPOIs r_WH --parallel 16  #--job-mode=condor

        combineTool.py -M Impacts -d $WORKSPACE -m 125 -t -1 -o sm_mc_${ERA}_${CHANNEL}_impacts.json --setParameters r_WH=1.0 --setParameterRanges r_WH=-5,5 --redefineSignalPOIs r_WH
        ...
        combineTool.py -M Impacts -d $WORKSPACE -m 125 -o sm_mc_${ERA}_${CHANNEL}_impacts.json --setParameters r_WH=1.0 \
            --setParameterRanges r_WH=-5,5 \
            --redefineSignalPOIs r_WH
        plotImpacts.py -i sm_mc_${ERA}_${CHANNEL}_impacts.json -o sm_mc_${ERA}_${CHANNEL}_impacts #--blind
...
if [[ $MODE == "UNC_SPLIT" ]]; then
    ...
    for POI in ${POIS[@]}; do
        combine -M MultiDimFit $DATACARD_OUTPUT/workspace.root -n .snapshot -m 125 --saveWorkspace -t -1 --redefineSignalPOIs $POI --setParameters r_WH=1.0 --robustFit 1 \
                --X-rtd MINIMIZER_analytic \
                --cminDefaultMinimizerStrategy 0 \

        combine -M MultiDimFit higgsCombine.snapshot.MultiDimFit.mH125.root -n .nominal -m 125 -t -1 --algo grid --snapshotName MultiDimFit --redefineSignalPOIs $POI --points 20 --setParameters r_WH=1.0 --robustFit 1 \
                --X-rtd MINIMIZER_analytic \
                --cminDefaultMinimizerStrategy 0 \
        ...
        combine -M MultiDimFit higgsCombine.snapshot.MultiDimFit.mH125.root -n .freezeall -m 125 -t -1 --algo grid --freezeParameters allConstrainedNuisances --snapshotName MultiDimFit --redefineSignalPOIs $POI --points 20 --setParameters r_WH=1.0 --robustFit 1 \
                --X-rtd MINIMIZER_analytic \
                --cminDefaultMinimizerStrategy 0 \
...
combineTool.py -M MultiDimFit \
    ...
    --algo grid \
    --points 50 \
    --setParameters cross_section_VV=0 \
    --setParameterRanges cross_section_VV=-10,10 \
