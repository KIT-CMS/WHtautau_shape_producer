...
FF="jetfakes_wpVSjet_VTight_07_06_24"
NN="nn_test_3"
...
do
    ...
    do
        ...
        if [ $ANALYSIS == "cut" ]; then
            if [ $REGION == "control" ]; then
                python shapes/produce_shapes.py --channels ${CHANNEL} --output-file ${OUTPUT_FILE} --directory ${NTUPLE_PATH}  --era ${ERA} --num-processes 2 --num-threads 2 --optimization-level 1  --ntuple_type crown --${CHANNEL}-friend-directory ${FRIEND_PATH}/crosssection  ${FRIEND_PATH}/${FF} --control-plot-set m_tt  --region ${REGION} --xrootd --skip-systematic-variation --control-plots #--process-selection d,wh_htt_minus #,m_vis,mjj,njets,pt_vis,nbtag,pt_W,pt_1,pt_2,pt_3
                # # # #jetfakes estimation
            ...
            python shapes/produce_shapes.py --channels ${CHANNEL} --output-file ${OUTPUT_FILE} --directory ${NTUPLE_PATH}  --era ${ERA} --num-processes 2 --num-threads 2 --optimization-level 1  --ntuple_type crown --${CHANNEL}-friend-directory ${FRIEND_PATH}/crosssection  ${FRIEND_PATH}/${FF} ${FRIEND_PATH}/${NN} --control-plot-set mmt_max_score  --region ${REGION} --xrootd #--skip-systematic-variation 
            bash shapes/do_estimations.sh ${ERA} ${OUTPUT_FILE}.root 0 
        fi
    done
